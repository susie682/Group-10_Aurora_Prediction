...
# -------------------- Paths & Columns --------------------
CSV_PATH = "final-24.csv"           # Read directly from the current directory
TARGET_COL = "keogram_mean"
DROP_COLS = {"time", "keogram_mean", "keogram_median", "keogram_max", "keogram_origin"}
EXCLUDE_KP_AP = {"kp", "ap", "kp_index", "ap_index", "kpindex", "apindex"}
...
# -------------------- [Optional] Filter by satellite_origin == 1 --------------------
USE_SATELLITE_ORIGIN_EQ1 = True  

if USE_SATELLITE_ORIGIN_EQ1:
    if "satellite_origin" not in df.columns:
        raise ValueError("Column 'satellite_origin' not found; cannot apply the optional filter.")
    before_n = df.height
    df = df.filter(pl.col("satellite_origin") == 1)
    print(f"Applied satellite_origin==1 filter: {before_n} -> {df.height}")
# -------------------------------------------------------------------------------
...
print("\nSaved: best_cnn.pt, best_config.json, best_metrics.json, tuning_results.csv")
