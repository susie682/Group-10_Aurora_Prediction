...
# ================= TEST — Time Series: Actual vs. Predicted（仅两条线） =================
# （为确保时间顺序正确，这里按时间排序再画）
ord_test = np.argsort(t_test)
plt.figure(figsize=(12, 4.7))
plt.plot(t_test[ord_test], y_test[ord_test], lw=1, label="Actual")
plt.plot(t_test[ord_test], test_pred[ord_test], lw=1, label="Predicted")
plt.xlabel("Time"); plt.ylabel(TARGET_COL)
plt.title("TEST — Time Series")
plt.legend()
plt.tight_layout()
_savefig(os.path.join(OUT_DIR, "test_timeseries_actual_vs_pred.png")); plt.close()
