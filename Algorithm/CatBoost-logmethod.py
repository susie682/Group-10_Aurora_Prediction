...
def event_metrics(y_true_raw, y_pred_raw, tag):
    y_true_evt = to_event_labels(y_true_raw, extreme_thr)
    y_pred_evt = to_event_labels(y_pred_raw, extreme_thr)
    prec = precision_score(y_true_evt, y_pred_evt, zero_division=0)
    rec  = recall_score(y_true_evt, y_pred_evt, zero_division=0)
    f1   = f1_score(y_true_evt, y_pred_evt, zero_division=0)
    print(f"[{tag}] Extreme events -> Precision: {prec:.6f} | Recall: {rec:.6f} | F1: {f1:.6f}")
    return prec, rec, f1

event_metrics(y_va_raw, va_pred_raw, "VAL (2018)")
event_metrics(y_te_raw, te_pred_raw, "TEST (2019–2020)")
