...
def dtw_distance(s, t, window_ratio=None):
    ...
    Args:
        s, t: 1D arrays (cast to float64).
        window_ratio: same as window_frac in your classmate's code.
                      Fraction of max(len(s), len(t)) used as band width.
                      If None -> full matrix.

    Returns:
        Normalized DTW distance: total alignment cost divided by (n + m).
    ...
    for i in range(1, n + 1):
        ...
        for j in range(j_start, j_end + 1):
            cost = abs(si - t[j - 1])  # L1 point-wise cost
            D[i, j] = cost + min(
                D[i - 1, j],     # insertion
                D[i, j - 1],     # deletion
                D[i - 1, j - 1]  # match
...
pos_rate_train = float((y_train_raw_wz >= extreme_thr).mean())  # p = 1 - EXTREME_Q（更稳：用实际比例）
