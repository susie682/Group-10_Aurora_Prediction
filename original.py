...
def save_grid(slices, hour_ranges, out_path):
    """
    Save a grid preview showing all segments.
    Layout adapts automatically:
      - 8 slices -> 2×4
      - 24 slices -> 3×8
      - otherwise tries a nearly square grid
    """
    import math
    import matplotlib.pyplot as plt
    ...
    for i, (ax, section) in enumerate(zip(axes, slices)):
        ...
        ax.axis("off")
        sh, eh = hour_ranges[i]
        ax.set_title(f"{int(sh)}–{int(eh)} h", fontsize=8)

    # Hide unused subplots if grid not perfectly filled
...
def run_all_years():
    ...
    for year in range(2021, 2025):
        input_dir = base / f"keogram-out{year}"
        output_csv = base / f"keogram_segment_stats{year}_24hours.csv"

        if not input_dir.exists():
            print(f"[skip] {input_dir} not found")
            continue

        print(f"[run] {year}: {input_dir} -> {output_csv}")
...
if __name__ == "__main__":
    # run_all_years() #trigger of old method

    # process_all(input_dir="keogram-out2", output_dir="segments-out", n_sections=8)
    # use to get segment directory
    process_all_with_csv(
        input_dir="keogram-out2021",
        output_dir="segments-out2021",
        output_csv="keogram_segment_stats2021.csv",
        n_sections=24
    )

    write_segment_stats_csv_with_red_filter(
            input_dir="keogram-out2021",
            output_csv="keogram_segment_stats2021_25%moon.csv",
            n_sections=8,
            red_ratio_threshold=0.25,   # your 50% rule
            require_min_valid_frac=0.30,
            use_invalid_mask=True
        )